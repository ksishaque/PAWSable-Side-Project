    ...
    public static class MiscellaniousExtensions {
        ...
        public static Rect Union(this Rect rect, params Rect[] other) {
            if (other == null || other.Length == 0){
                ...
                var xMin = Mathf.Min(rect.xMin, other.Select(x => x.xMin).Aggregate(Mathf.Min));
                var yMin = Mathf.Min(rect.yMin, other.Select(x => x.yMin).Aggregate(Mathf.Min));
                var xMax = Mathf.Max(rect.xMax, other.Select(x => x.xMax).Aggregate(Mathf.Max));
                var yMax = Mathf.Max(rect.yMax, other.Select(x => x.yMax).Aggregate(Mathf.Max));
